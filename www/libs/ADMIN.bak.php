...
class ADMIN extends DB
{
    ...
    function register($data){    
        ...
        $sql = "SELECT SUM(case when login = ? then 1 else 0 end), SUM(case when mail = ? then 1 else 0 end) FROM accounts WHERE login = ? OR mail = ?";
        $stmt = $db->prepare($sql);
        $stmt->bind_param('ssss', $data->username, $data->email,$data->username, $data->email);
		$stmt->execute();
        $stmt->bind_result($username_count, $email_count);        
        while($stmt->fetch()) {                        
            //        
        }
        if($username_count == 1 || $email_count == 1){        
            if($username_count == 1 && $email_count == 1) $result['success'] = 1;
            else if($username_count == 1) $result['success'] = 2;
            else if($email_count == 1) $result['success'] = 3;        
        }else{
            $stmt->reset();
            $stmt->close();
            ...
            if(!mail($data->email, "hackweb - registrazione", $mex, $headers)){
                ...
                $stmt = $db->prepare($sql);
                $stmt->bind_param('ssss', $data->username, $data->password, $data->email, $cod_verifica);                
                if(!$stmt->execute()) $result['success'] = 5;    
            }
        }            
    ...
    function verificaAccount($data){
        ...
        $stmt = $db->prepare($sql);
        $stmt->bind_param('ss', $data->username, $data->cod_verifica);
        $stmt->execute();
        //$stmt->store_result();
        $stmt->bind_result($verifica);
        $i=0;        
        while($stmt->fetch()) {                        
            $i++;
        }
        if($i==0){
            ...
            if($verifica == 0){
                ...
                $stmt = $db->prepare($sql);
                $stmt->bind_param('ss', $data->username, $data->cod_verifica);             
                if(!$stmt->execute()) $result['success'] = 1;  
            }else{
                $result['success'] = 2;
            }            
        }        
    ...
    function login($data){
        ...
        $stmt->execute();
        //$stmt->store_result();
        $stmt->bind_result($potere, $verifica, $sess_id);
        $i=0;
        $result = array(
            'success'       => 0
        );
        ...
        if($i==1){
            if($verifica == 0){
                ...
                if($sess_id == null){
                    $sess_id = $this->proteggi('d',$data->username,rand(),"gga",97);
                }
                ...
                if(isset($data->ricordami)){
                    $result['success'] = 2;
                    setcookie($this->proteggi("g","giovi","sess_id","dfg","115"), $sess_id, time()+3000000, "/");                    
                }                
            }                        
        }
        echo json_encode($result);
    ...
    function checkLogin(){   
        ...
        if(isset($_SESSION['sess_id']) || isset($_COOKIE[$this->proteggi("g","giovi","sess_id","dfg","115")])){
            ...
            // $stmt->bind_result($username, $potere);
            $i=0;
            
            if($i==1){
                $result['username'] = $_SESSION['username'] = $username;
                $result['potere'] = $_SESSION['potere'] = $potere;
