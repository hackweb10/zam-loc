...
class ADMIN extends DB
{
    ...
    function register($data){    
        ...
        $db = $this->conn;        
        $result = ['success' => 0];
        $sql = "SELECT SUM(case when login = ? then 1 else 0 end) AS login_count, SUM(case when mail = ? then 1 else 0 end) AS mail_count FROM accounts WHERE login = ? OR mail = ?";
        $stmt = $db->prepare($sql);        
		$stmt->execute([$data->username, $data->email,$data->username, $data->email]);
        $ob = $stmt->fetchObject();
        $i = $stmt->rowCount();        
        if($ob->login_count == 1 || $ob->mail_count == 1){        
            if($ob->login_count == 1 && $ob->mail_count == 1) $result['success'] = 1;
            else if($ob->login_count == 1) $result['success'] = 2;
            else if($ob->mail_count == 1) $result['success'] = 3;        
        }else{                           
            $codice = rand();            
            $cod_verifica = md5($codice);
            ...
            --------------------------<br><br>
            Messaggio automatico, non rispondere.<br><br>";
            ...
            -----------------------------
            <br>
            nick:		".$data->username."<br>
            mail:		".$data->email."<br>
            ip:		    ".$_SESSION['USER_IP']."<br>
            codice:     ".$cod_verifica."<br>
            ------------------------------
            <br><br>
            ...
            if(!mail($data->email, "hackweb - registrazione", $mex, $headers)){
                ...
                $stmt = $db->prepare($sql);                
                if(!$stmt->execute([$data->username, $data->password, $data->email, $cod_verifica])) $result['success'] = 5;                    
            }
        }                    
        echo json_encode($result);        
    }

    function verificaAccount($data){
        //connection
        $db = $this->conn;        
        $sql = "SELECT verifica FROM accounts WHERE login = ? AND cod_verifica = ?";    
        $result = ['success' => 0];
        $stmt = $db->prepare($sql);        
        $stmt->execute([$data->username, $data->cod_verifica]);
        $ob = $stmt->fetchObject();
        $i = $stmt->rowCount();
        if($i==0){
            $result['success'] = 1;
            ...
            if($ob->verifica == 0){                
                $sql = "UPDATE accounts SET verifica = '1' WHERE login = ? AND cod_verifica = ?";
                $stmt = $db->prepare($sql);
                if(!$stmt->execute([$data->username, $data->cod_verifica])) $result['success'] = 1;                
            }else{
                $result['success'] = 2;
            }            
        }        
    ...
    function login($data){
        $data->password = md5($data->password);
        // $data->encry = ($this->proteggi('g', $data->username, 'xwp', 'swk', '947'));
        setcookie($this->proteggi("g","giovi","sess_id","dfg","115"), '', time()-3600, "/");        
        unset($_COOKIE[$this->proteggi("g","giovi","sess_id","dfg","115")]);
        
        //connection
        $db = $this->conn;        
        //check potere
        $sql = "SELECT potere, verifica, sess_id FROM accounts WHERE BINARY password = ? AND BINARY login = ?";
        $stmt = $db->prepare($sql);                            
        ...
        if($i==1){
            if($ob->verifica == 0){
                ...
                if($ob->sess_id == null){
                    $ob->sess_id = $this->proteggi('d',$data->username,rand(),"gga",97);
                }
                $_SESSION['sess_id'] = $ob->sess_id;
                $_SESSION['username'] = $data->username;
                $_SESSION['potere'] = $ob->potere;
                $sql = 'UPDATE accounts SET sess_id=? WHERE password=? AND login=?';
                $stmt = $db->prepare($sql);
                $stmt->execute([$ob->sess_id, $data->password, $data->username]);                            
                $result['success'] = 1;
                $result['utente'] = $data->username;
                if(isset($data->ricordami)){
                    $result['success'] = 2;
                    $result['cookie_name'] = $this->proteggi("g","giovi","sess_id","dfg","115");
                    $result['cookie_value'] = $ob->sess_id;
                    setcookie($this->proteggi("g","giovi","sess_id","dfg","115"), $ob->sess_id, time()+3000000, "/", '', true, true);      
                    // $_COOKIE[$this->proteggi("g","giovi","sess_id","dfg","115")] = $ob->sess_id;                    
                }                
            }                        
        }
        echo json_encode($result);        
    ...
    function logout($data){        
        $result = array(
            'success'       => 0,
            'cookie_name'   => $this->proteggi("g","giovi","sess_id","dfg","115")            
        );
        if(isset($_COOKIE[$this->proteggi("g","giovi","sess_id","dfg","115")])) $result['success'] = 2;
        setcookie($this->proteggi("g","giovi","sess_id","dfg","115"), '', time()-3600, "/");                
        unset($_COOKIE[$this->proteggi("g","giovi","sess_id","dfg","115")]);
        $_SESSION['sess_id'] = null;
        $_SESSION['username'] = null;
        $_SESSION['potere'] = null;        
        echo json_encode($result);
    }
    ...
    function checkLogin(){           
        ...
        if(isset($_SESSION['sess_id']) || isset($_COOKIE[$this->proteggi("g","giovi","sess_id","dfg","115")])){
            $sess_id = (isset($_COOKIE[$this->proteggi("g","giovi","sess_id","dfg","115")])) ? $_COOKIE[$this->proteggi("g","giovi","sess_id","dfg","115")] : $_SESSION['sess_id'];
            $db = $this->conn;            
            $sql = "SELECT login, potere FROM accounts WHERE sess_id = ?";
            $stmt = $db->prepare($sql);            
            $stmt->execute([$sess_id]);
            $ob = $stmt->fetchObject();            
            $i = $stmt->rowCount();            
            if($i==1){
                $result['username'] = $_SESSION['username'] = $ob->login;
                $result['potere'] = $_SESSION['potere'] = $ob->potere;
    ...
    function checkToken($token){           
        ...
        $i = $stmt->rowCount();            
        if($i==1){
            $result['username'] = $ob->login;
            $result['potere'] = $ob->potere;            
        }            
        return $result;
    }
    ...
    function getNews(){		
		$db = $this->conn;
		$sql = "SELECT data, testo FROM news ORDER BY id DESC LIMIT 1";
		$stmt = $db->prepare($sql);		
        $stmt->execute([]);
        $i = $stmt->rowCount();    
        // $ob = $stmt->fetchAll(PDO::FETCH_OBJ);            
        // foreach($ob as $key => $value) {
        //     echo('- <span class="color-1">'.$key.'</span>:<br>'.$value.'<br><br>');
        // }
        while($ob = $stmt->fetchObject()){
            echo('<div><span class="color-1">'.$ob->data.'</span><br>'.$ob->testo.'</div>');
        }        
    }
    
    //add news
    function addNews($data){
        $verifica = $this->checkLogin();
        $result = ['success' => 1, 'potere' => $verifica['potere']];
        if($verifica['potere'] == 100){
            $date = date('d/m/y');
            $db = $this->conn;
            $sql = "INSERT INTO news (data, testo) VALUES (?, ?)";
            $stmt = $db->prepare($sql)->execute([$date, $data->testo]);
            $result['success'] = 0;
        }        
        echo json_encode($result);
    }
    ...
    function test($data){
        // $result = [
        //     ['id' => 1, 'name' => 'scoreggia', 'description' => 'marcia'],
        //     ['id' => 2, 'name' => 'pup', 'description' => 'marcia']
        // ];

        if(isset($data->token)) $this->checkToken($data->token);

        $result = ['id' => 1, 'name' => 'scoreggia', 'description' => 'marcia'];
    ...
    function getTranslation($data){                    
        ...
        foreach ($data->lang as $value) {
            $sql = "SELECT name, text, cat FROM hw_translations WHERE lang = ?";            
            $stmt = $db->prepare($sql);        
            $stmt->execute([$value]);        
            $i = $stmt->rowCount();
            while($ob = $stmt->fetchObject()){                            
                $result[$value][$ob->name] = $ob->text;
            }        	    
        }            
        echo json_encode($result);		    
    }	 
    ...
    function getBrands(){            
        $result = [];
        $db = $this->conn;
        $sql = 'SELECT id, nome, descrizione FROM marche ORDER BY nome';		        	
        $stmt = $db->prepare($sql);
        $stmt->execute([]);	        
        $i = $stmt->rowCount();    	            
        while($ob = $stmt->fetchObject()){            
